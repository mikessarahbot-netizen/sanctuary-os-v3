...
const createRepository = (executor: PlanningSqlExecutor) => {
  ...
  return createPlanningCcliUsageSqlRepository({
    ...
    ids: {
      ...
      ccliUsageLogId: () => {
        const ccliUsageLogId = `ccli_log_${String(nextCcliUsageLogId)}`;
        nextCcliUsageLogId += 1;
...
describe("Planning CCLI usage SQL repository", () => {
  it("records tenant-scoped CCLI usage logs with pending status and audit metadata", async () => {
    const executor = createRecordingExecutor([
      {
        ccli_song_number: "123456",
        ccli_usage_log_id: "ccli_log_1",
        notes: "Used in livestream.",
        ...
        used_at: "2026-06-14T14:30:00.000Z"
      }
    ]);
    ...
    await expect(
      repository.recordCcliUsage({
        input: {
          ...
          usedAt: "2026-06-14T14:30:00.000Z"
        },
        options: {
          context: {
            actorId: "actor_1",
    ...
    expect(insert.transaction).toEqual({ transactionId: "tx_1" });
    ...
    expectSqlContains(insert, "AND service_item.service_id = $3");
  ...
  it("uses supplied transactions without opening an independent CCLI write transaction", async () => {
  ...
  it("does not audit failed CCLI usage records with missing tenant-owned service rows", async () => {
    const executor = createRecordingExecutor([[]]);
    const repository = createRepository(executor);
    ...
    ).rejects.toThrow("Planning CCLI usage log record returned no row.");

    expect(executor.statements).toHaveLength(1);
  });

  it("lists CCLI usage logs with tenant, service, status, and transaction filters", async () => {
    ...
    expectParameters(statement, ["tenant_1", "service_1", "reported"]);
  });

  it("keeps vendor credentials and volunteer contact fields outside CCLI SQL", async () => {
    const executor = createRecordingExecutor([[]]);
    const repository = createRepository(executor);

    await repository.listCcliUsageLogs({
      input: {
        serviceId: "service_1"
      },
      options: {
    ...
    expect(statement.sql).not.toContain("api_key");
    expect(statement.sql).not.toContain("email");
    expect(statement.sql).not.toContain("phone");
  });

  it("rejects malformed CCLI rows before returning repository records", async () => {
    const executor = createRecordingExecutor([
      {
        ccli_usage_log_id: "ccli_log_1",
    ...
    await expect(
      repository.recordCcliUsage({
        ...
        options: {
          context: {
            requestId: "request_invalid_ccli_row",
            tenantId: "tenant_1"
          },
          intent: "create"
        }
