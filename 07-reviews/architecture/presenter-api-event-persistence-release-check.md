...
| In-memory persistence adapters | Pass | `packages/db/src/presenter-in-memory-repository.ts` implements separate query and command repositories for the new contracts. Tests cover tenant isolation, actor/request operation recording, transaction ID recording, theme/output/presentation/slide mutations, output target presentation association, defensive copies, and invalid scope/out-of-scope payload rejection. |
...
| Adapter isolation | Pass | Presenter DB contracts and shared in-memory persistence adapters live in `packages/db` without SQL, GraphQL, Auth0, event publishing, UI, OBS, vendor SDK, AI execution, deployment config, or checked-in secrets. API Presenter services keep their own in-memory service adapter separate and do not add production database wiring. |
...
## Validation
- `pnpm --filter @sanctuary-os/api test -- presenter`
- `pnpm --filter @sanctuary-os/db test -- presenter-in-memory-repository.test.ts presenter-repository-contracts.test.ts`
- `pnpm lint`
- `pnpm typecheck`
- `pnpm test`
