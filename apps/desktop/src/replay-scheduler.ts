 ...
 */
export interface PresenterDesktopReplayIntervalScheduler<THandle> {
  readonly cancel: (handle: THandle) => void;
  readonly schedule: (callback: () => void, intervalMs: number) => THandle;
}

export interface PresenterDesktopReplaySchedulerDependencies<TResult, THandle> {
...
export type PresenterDesktopReplayRunOutcome<TResult> =
  | { readonly status: "skipped-offline" }
  | { readonly result: TResult; readonly status: "ran" };

export interface PresenterDesktopReplayScheduler<TResult> {
  readonly runOnce: () => Promise<PresenterDesktopReplayRunOutcome<TResult>>;
  readonly start: () => void;
  readonly stop: () => void;
}

export const createPresenterDesktopReplayScheduler = <TResult, THandle>(
  dependencies: PresenterDesktopReplaySchedulerDependencies<TResult, THandle>
): PresenterDesktopReplayScheduler<TResult> => {
  let handle: THandle | undefined;

  const runOnce = async (): Promise<PresenterDesktopReplayRunOutcome<TResult>> => {
